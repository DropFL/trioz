...
public final class KeyStatus
{
    ...
    public static void register(JFrame frame)
    {
        ...
        {
            final Key key = k;
            component.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(key.getKeyCode(), 0),
                    key.toString() + " pressed");
            ...
            component.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW)
                    .put(KeyStroke.getKeyStroke(key.getKeyCode(), 0, true), key.toString() + " released");
    ...
    {
        if (isInitialized && status.get(key) != 0)
            status.put(key, 1);
    }
}
